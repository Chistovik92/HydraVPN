...
- Текущая версия: **0.6.13** (`app/build.gradle.kts` → `versionName`)
...
- **Фаза 7. Устойчивость рантайма** — отдельный трек, заведён по итогам аудита
  ...
  **План починки (порядок = порядок ценности, каждая подфаза
  ...
  - **7a. Наблюдаемость туннеля.** Расширить `VpnCore` колбэком
    `onDied: (reason: String) -> Unit` (дефолт `{}` — существующие ядра
    компилируются без правок) и прокинуть в него то, что сейчас уходит в
    лог: `onDown` PPP в `SstpCore`/`L2tpCore`, `onTunnelDown` в
    `L2tpTransport`, завершение `sstp-reader`/`l2tp-reader`. Для sing-box
    — использовать уже поднятый `CommandClient` (`SingBoxRuntime`): его
    `disconnected(message)` сейчас пустой, а это готовый сигнал «сервис
    отвалился». `HydraVpnService` подписывается и переводит состояние в
    `ERROR`, применяя ту же ветку Kill Switch, что и при неудачном
    подключении (вынести её из `doConnect()` в отдельный
    `enterKillSwitch()`). Критерий готовности: убитый на сервере туннель
    за ≤5 с меняет UI и уведомление, а не остаётся «зашифрованным».
...
11. **Наблюдаемость и переподключение туннеля** (Фаза 7a-7c в дорожной карте
    ...
    `VpnState.state`, переподключения нет. Делать строго по порядку: 7a
    (канал «ядро умерло» в `VpnCore`) → 7b (бэкофф + `RECONNECTING` +
    Kill Switch на всё время) → 7c (`NetworkCallback` и
