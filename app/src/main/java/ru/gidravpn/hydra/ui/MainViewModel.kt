...
class MainViewModel(app: Application) : AndroidViewModel(app) {
    /**
     * Все корутины вьюмодели идут через safeLaunch: у viewModelScope нет своего
     * обработчика, и любое исключение из Room/DataStore (переполненный диск,
     * повреждённая база) роняло процесс прямо из фонового сохранения настройки.
     * Теперь оно попадает в журнал, а приложение продолжает работать.
     */
    private fun safeLaunch(
        ...
        try {
            ...
            VpnState.log("Ошибка: ${e.message ?: e.javaClass.simpleName}")
        }
    }
    ...
    // ----- Фаза 8: IPv6, профили маршрутизации, приватность, обновления -----

    private val privacyRepo = ru.gidravpn.hydra.data.repository.PrivacyRepository(app)
    private val profilesRepo = ru.gidravpn.hydra.data.repository.RoutingProfilesRepository(app)

    val ipv6Mode: StateFlow<ru.gidravpn.hydra.data.model.Ipv6Mode> = routingRepo.ipv6Mode
        .stateIn(viewModelScope, SharingStarted.Eagerly, ru.gidravpn.hydra.data.model.Ipv6Mode.BLOCK)
    fun setIpv6Mode(mode: ru.gidravpn.hydra.data.model.Ipv6Mode) = safeLaunch { routingRepo.setIpv6Mode(mode) }

    val routingProfiles: StateFlow<List<ru.gidravpn.hydra.data.repository.RoutingProfilesRepository.Profile>> =
        profilesRepo.profiles.stateIn(viewModelScope, SharingStarted.Eagerly, emptyList())
    fun saveRoutingProfile(name: String) = safeLaunch {
        if (name.isBlank()) return@safeLaunch
        profilesRepo.saveCurrent(name)
        VpnState.log("Профиль маршрутизации «${name.trim()}» сохранён")
    }
    /** Применить профиль; при поднятом туннеле — переподключение, как при правке split-правил. */
    fun applyRoutingProfile(p: ru.gidravpn.hydra.data.repository.RoutingProfilesRepository.Profile) = safeLaunch {
        profilesRepo.apply(p)
        VpnState.log("Профиль маршрутизации «${p.name}» применён")
        reapplySplitIfConnected()
    }
    fun deleteRoutingProfile(name: String) = safeLaunch { profilesRepo.delete(name) }

    val appLock: StateFlow<Boolean?> = privacyRepo.appLock.map<Boolean, Boolean?> { it }
        .stateIn(viewModelScope, SharingStarted.Eagerly, null)
    fun setAppLock(enabled: Boolean) = safeLaunch { privacyRepo.setAppLock(enabled) }

    val hideSecrets: StateFlow<Boolean> = privacyRepo.hideSecrets
        .stateIn(viewModelScope, SharingStarted.Eagerly, true)
    fun setHideSecrets(enabled: Boolean) = safeLaunch { privacyRepo.setHideSecrets(enabled) }

    val updateCheckEnabled: StateFlow<Boolean> = privacyRepo.updateCheck
        .stateIn(viewModelScope, SharingStarted.Eagerly, true)
    fun setUpdateCheck(enabled: Boolean) = safeLaunch { privacyRepo.setUpdateCheck(enabled) }

    /** Состояние проверки обновлений для экрана «О приложении» и плашки на главном. */
    sealed interface UpdateUi {
        data object Idle : UpdateUi
        data object Checking : UpdateUi
        data object UpToDate : UpdateUi
        data class Available(val release: ru.gidravpn.hydra.data.update.UpdateChecker.Release) : UpdateUi
    ...
    fun checkForUpdates(manual: Boolean) = safeLaunch {
        if (!manual) {
            if (privacyRepo.updateCheck.first() != true) return@safeLaunch
            val last = privacyRepo.lastUpdateCheck.first()
            if (System.currentTimeMillis() - last < 24 * 3_600_000L) return@safeLaunch
        }
        _update.value = UpdateUi.Checking
        _update.value = runCatching { ru.gidravpn.hydra.data.update.UpdateChecker.latest() }.fold(
            onSuccess = { r ->
                privacyRepo.markUpdateChecked(System.currentTimeMillis())
                if (ru.gidravpn.hydra.data.update.UpdateChecker.isNewer(r.version, ru.gidravpn.hydra.BuildConfig.VERSION_NAME))
                    UpdateUi.Available(r)
                else if (manual) UpdateUi.UpToDate else UpdateUi.Idle
            },
            onFailure = { if (manual) UpdateUi.Failed(it.message ?: it.javaClass.simpleName) else UpdateUi.Idle },
        )
    }

    /** Системный Always-on VPN: (включён, блокировка без VPN) — из работающего сервиса. */
    val alwaysOn = VpnState.alwaysOn
